...
public class StoresAapter extends RecyclerView.Adapter<StoresAapter.ItemViewHolder> {
    private final List<Stores> list;
    private Context mContext;
    ForthBookActivity.StoreListener bookSelectedListener;


    public StoresAapter(ArrayList<Stores> list, ForthBookActivity mContext, ForthBookActivity.StoreListener bookSelectedListener) {
        this.list = list;
    ...
    public ItemViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new ItemViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_stores, parent, false));
    }

    @Override
    public void onBindViewHolder(ItemViewHolder holder, final int position) {
        holder.textview_address.setText(list.get(position).getStore_name()+", "+list.get(position).getStreet_address()+", "+list.get(position).getCity()+", "+list.get(position).getState()+", "+list.get(position).getCountry());
